...
import { TodayActivityCard } from "@/components/guest/today-activity-card";
import { AnnouncementCard } from "@/components/guest/announcement-card";
import { SectionHeader } from "@/components/guest/section-header";
import { quickActions, todayActivities } from "@/lib/demo-data";
  ...
  return <main className="min-h-screen bg-[#f7f4ee] pb-24 md:pb-0">
    <section className="relative min-h-[610px] overflow-hidden bg-[#123f3e] text-white md:min-h-[650px]">
      ...
      <div className="relative z-10 mx-auto flex min-h-[610px] max-w-7xl items-end px-5 pb-24 pt-32 md:min-h-[650px] md:px-8"><div className="max-w-xl">
        ...
        <p className="mt-6 max-w-md text-lg leading-relaxed text-white/78">Tout votre séjour, directement depuis votre téléphone.</p>
      </div></div>
    </section>
    <div className="mx-auto max-w-7xl px-5 py-12 md:px-8 md:py-20">
      ...
      <section className="mt-16"><SectionHeader eyebrow="Vendredi 12 septembre" title="Aujourd’hui à l’hôtel" href="/activities"/><div className="scrollbar-none flex gap-3 overflow-x-auto pb-2">{todayActivities.map(a=><TodayActivityCard key={a.title} activity={a}/>)}</div></section>
      <section className="mt-16"><AnnouncementCard/></section>
    </div><MobileBottomNav/>
  </main>;
}
